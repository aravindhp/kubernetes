...
func validateServices(services []string) field.ErrorList {
	...
	if totalLength > maxTotalServiceNamesLength {
		allErrs = append(allErrs, field.TooLongMaxLength(field.NewPath("query"), services, maxTotalServiceNamesLength))
	}
	return allErrs
}

func (n *nodeLogQuery) validate() field.ErrorList {
	// This is to account for the case that query is empty and the caller is directly trying to query a file
	// Example: kubectl get --raw /api/v1/nodes/$node/proxy/logs/foo.log
	if len(n.Services) == 0 && len(n.Files) == 0 && n.options == (options{}) {
		return field.ErrorList{}
	}

	allErrs := validateServices(n.Services)
	switch {
	case len(n.Files) == 0 && len(n.Services) == 0 && n.options != (options{}):
	...
	if n.TailLines != nil {
		if err := utilvalidation.IsInRange((int)(*n.TailLines), 0, 100000); err != nil {
			allErrs = append(allErrs, field.Invalid(field.NewPath("tailLines"), *n.TailLines, err[0]))
		}
	}

	if err := safeString(n.Pattern); err != nil {
...
func heuristicsCopyFileLogs(ctx context.Context, w io.Writer, service string) {
	logDir := "/var/log/"
	logFiles := [7]string{
		filepath.Join(logDir, fmt.Sprintf("%s", service)),
		filepath.Join(logDir, fmt.Sprintf("%s.log", service)),
		filepath.Join(logDir, service, fmt.Sprintf("%s.log", service)),
		filepath.Join(logDir, fmt.Sprintf("%s*INFO.*", service)),
		filepath.Join(logDir, fmt.Sprintf("%s*INFO", service)),
		filepath.Join(logDir, service, fmt.Sprintf("%s*INFO.*", service)),
		filepath.Join(logDir, service, fmt.Sprintf("%s*INFO", service)),
	}

	var err error
	for _, logFile := range logFiles {
		if err = heuristicsCopyFileLog(w, logFile); err == nil {
			break
		}
	}

	if err != nil {
		// If the last error was file not found it implies that no log files were for found for the service
...
func heuristicsCopyFileLog(w io.Writer, logFile string) error {
