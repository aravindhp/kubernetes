...
func getLoggingCmd(n *nodeLogQuery, services []string) (string, []string, error) {
	...
	if n.SinceTime != nil {
		args = append(args, fmt.Sprintf("--since=%s", strings.TrimSuffix(n.SinceTime.UTC().String(), " +0000 UTC")))
	}
	if n.UntilTime != nil {
		args = append(args, fmt.Sprintf("--until=%s", strings.TrimSuffix(n.UntilTime.UTC().String(), " +0000 UTC")))
	}
	if n.TailLines != nil {
