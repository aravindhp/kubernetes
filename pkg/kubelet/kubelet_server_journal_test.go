...
func Test_validateServices(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "one service", services: []string{"foo"}},
		{name: "two services", services: []string{"foo", "bar"}},
		{name: "long service", services: []string{strings.Repeat(service1, 100)}, wantErr: true},
		{name: "max service length", services: []string{strings.Repeat(service1, 100), strings.Repeat(service2, 100)},
			wantErr: true},
	}
	for _, tt := range tests {
...
func Test_nodeLogQuery_validate(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "one file", Files: []string{file1}},
		...
		{name: "unsafe pattern", Services: []string{service1}, options: options{Pattern: unsafe}, wantErr: true},
