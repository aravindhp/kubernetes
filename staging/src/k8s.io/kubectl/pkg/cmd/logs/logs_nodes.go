...
func NewNodeLogsCmdFlags(restClientGetter genericclioptions.RESTClientGetter, streams genericclioptions.IOStreams) *NodeLogsCmdFlags {
...
func NewCmdNodeLogs(restClientGetter genericclioptions.RESTClientGetter, streams genericclioptions.IOStreams) *cobra.Command {
	flags := NewNodeLogsCmdFlags(restClientGetter, streams)
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			o, err := flags.ToOptions(args, cmd.Flags().Changed("boot"), cmd.Flags().Changed("path"))
...
func (flags *NodeLogsCmdFlags) ToOptions(args []string, bootChanged bool, pathSpecified bool) (*NodeLogsCmdOptions,
	error) {
	...
	if o.Object == nil {
		builder := resource.NewBuilder(flags.RESTClientGetter).
		...
		if len(flags.Role) > 0 {
			req, err := labels.NewRequirement(fmt.Sprintf("node-role.kubernetes.io/%s", flags.Role), selection.Exists, nil)
			if err != nil {
				return nil, fmt.Errorf("invalid --role: %v", err)
			}
			flags.Selector = req.String()
		}
		if len(flags.Selector) > 0 {
			o.Selector = flags.Selector
		...
		if flags.Selector == "" && len(infos) != 1 {
			return nil, fmt.Errorf("expected a node resource")
		}

		o.Object = infos[0].Object
		if flags.Selector != "" && len(o.Object.(*corev1.NodeList).Items) == 0 {
			return nil, fmt.Errorf("no node resources found")
		}
	}
...
func (o NodeLogsCmdOptions) Run() error {
	// convert to API
	options := &corev1.NodeLogQueryOptions{
	...
	var err error
	requests, err := o.LogsForObject(o.RESTClientGetter, o.Object, options, o.GetNodeTimeout, false)
	if err != nil {
		return err
	}
